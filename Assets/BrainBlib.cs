...
public class BrainBlib : Agent
{
  
RayPerceptionSensorComponent2D thisRay;
Rigidbody2D rb;
BrainBlibControls bctrl;
...
void Start() 
{
    ...
    bctrl = gameObject.GetComponent<BrainBlibControls>();
    energy = bctrl.energy;
...
sensor.AddObservation(bctrl.bump);
...
hasReproduced = bctrl.hasReproduced;
 ...
 bctrl.energy -=  Mathf.Abs(fwd.magnitude);
     ...
     if (booper.tag == "Predator" || booper.tag == "Predator2")
        {
            ...
            EndEpisode();
             ...
             if (booper.tag == "Prey" && energy >= bctrl.energyToReproduce*0.75f)
            {
